...
class AstsReportController extends Controller
{
    ...
    public function resetRanks(Request $request)
    {
        ...
        // 1. Hapus nomor peringkat manual
        AstsReport::whereIn('student_id', $studentIds)
            ->where('academic_year_id', $academicYearId)
            ->where('semester', $semester)
            ->update(['manual_rank' => null]);

        // 2. Kembalikan nilai ke nilai murni ujian (raw / nilai asli) jika ada paket ujian terkait
        $targetClass = ClassRoom::find($classId);
        $exams = ExamPackage::where('class_room_id', $classId)
            ->where(function ($q) use ($semester) {
                $q->where('exam_type', 'sts')
            ...
            try {
                foreach ($exams as $exam) {
                    foreach ($exam->submissions as $sub) {
                        ...
                        // Pulihkan ke skor murni pengerjaan ujian
                        $rawScore = round(floatval($sub->total_score));
        ...
        $msgDetail = $scoresRestored > 0
            ? " dan {$scoresRestored} nilai mata pelajaran telah dikembalikan 100% murni sesuai skor ujian asli!"
            : "!";

        return response()->json([
            'status' => 'success',
            'message' => 'Peringkat siswa telah dikembalikan ke otomatis' . $msgDetail,
