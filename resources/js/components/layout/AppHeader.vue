<template>
  <header class="h-16 flex items-center justify-between px-6 border-b border-slate-200 bg-white sticky top-0 z-30 shadow-2xs no-print">
    <div class="flex items-center gap-3">
      <button 
        @click="$emit('toggle-sidebar')"
        class="p-2 -ml-2 rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors cursor-pointer border border-transparent hover:border-slate-200"
      >
        <Menu class="w-5 h-5" />
      </button>
      <div class="flex items-center gap-2">
        <span class="w-2 h-2 rounded-full bg-emerald-600 hidden sm:inline-block"></span>
        <h2 class="text-base sm:text-lg font-bold text-slate-900 tracking-tight font-sans">
          {{ currentRouteName }}
        </h2>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2 sm:gap-2.5">
      ...
      <div class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-200 text-slate-700 text-xs font-semibold shadow-2xs font-mono tabular-nums">
      ...
      <div v-if="auth.isDualRole" class="flex items-center">
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-semibold shadow-2xs transition-colors cursor-pointer border border-emerald-950/40"
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-900 text-white text-xs font-semibold shadow-2xs transition-colors cursor-pointer border border-slate-950/40"
        ...
        >
          ...
          <ArrowRightLeft class="w-3 h-3 text-slate-300 opacity-80" />
        </button>
      </div>
      ...
      <div v-if="auth.role === 'admin' || auth.role === 'teacher'" class="relative">
        <button
          ...
          class="w-9 h-9 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-700 flex items-center justify-center relative transition-colors cursor-pointer border border-slate-200 shadow-2xs"
        >
          <Bell class="w-4 h-4" />
          <span
