<template>
  <div class="space-y-6 font-inter text-slate-800">
    ...
    <div v-if="activeAdminTab === 'applicants'" class="space-y-6">
      ...
      <!-- Applicants Data: Desktop Table & Mobile Cards -->
      <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        <div class="md:hidden divide-y divide-slate-100">
          ...
          <div v-else-if="applicants.length === 0" class="p-8 text-center text-xs text-slate-400">
            Belum ada calon siswa pada kriteria ini.
          </div>
          <div
          ...
          >
            ...
            <div class="flex items-start justify-between gap-2">
              <div class="flex items-center gap-3 min-w-0">
                <div class="w-11 h-11 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center flex-shrink-0 text-emerald-700 font-bold overflow-hidden">
                  <img v-if="app.photo_url" :src="app.photo_url" class="w-full h-full object-cover" />
                  <span v-else>{{ app.full_name?.charAt(0) }}</span>
                </div>
                <div class="min-w-0">
                  <h4 class="font-bold text-slate-900 text-sm truncate" @click="openDetail(app)">
                    {{ app.full_name }}
                  </h4>
                  <p class="text-[11px] font-mono text-slate-500">{{ app.registration_number }} &bull; {{ app.gender === 'L' ? 'L' : 'P' }}</p>
                </div>
              </div>
              
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border flex-shrink-0" :class="app.status_badge_class">
                {{ app.status_label }}
              </span>
            </div>

            <!-- Card Info Grid -->
            <div class="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-xl text-slate-600">
              <div>
                <span class="text-slate-400 block text-[10px]">Asal Sekolah:</span>
                <span class="font-medium truncate block">{{ app.previous_school || '-' }}</span>
              </div>
              <div>
                <span class="text-slate-400 block text-[10px]">Nilai Seleksi:</span>
                <span class="font-bold text-emerald-700">{{ app.test_score !== null ? app.test_score : '-' }}</span>
              </div>
              <div class="col-span-2 flex items-center justify-between pt-1 border-t border-slate-200/60">
                <span class="text-[11px] text-slate-500 font-mono">{{ app.phone }}</span>
                <a
                  v-if="app.phone"
                  :href="`https://wa.me/${cleanPhone(app.phone)}?text=Halo%20Bapak/Ibu%20Wali%20dari%20${encodeURIComponent(app.full_name)},%20kami%20dari%20Panitia%20PPDB%20MTs%20Al-Hasanah.`"
                  target="_blank"
                  class="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded-lg"
                >
            ...
            <div class="flex items-center gap-2 pt-1">
              <button
                @click="openDetail(app)"
                class="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition-all text-center cursor-pointer"
              >
                Review Berkas & Nilai
              </button>
              <button
                v-if="app.status === 'accepted'"
                @click="openEnrollModal(app)"
                class="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all text-center cursor-pointer flex items-center justify-center gap-1"
              >
                <UserCheck class="w-3.5 h-3.5" />
                <span>Masukkan Kelas</span>
              </button>
