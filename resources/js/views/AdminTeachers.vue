  ...
  <div class="space-y-5 font-inter">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-slate-900 tracking-tight font-sans">Data Pendidik & Tenaga Kependidikan</h1>
          <p class="text-xs text-slate-500 font-medium">Buku induk guru &bull; <span class="font-mono tabular-nums font-semibold text-slate-700">{{ totalRecords }}</span> guru terdaftar</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-2.5">
      ...
      <div class="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-600 shadow-2xs">
        <select v-model.number="selectedPerPage" class="bg-transparent border-none p-0 text-slate-800 font-semibold focus:ring-0 cursor-pointer text-xs pr-1" @change="onPerPageChange">
      ...
      <div class="relative flex-1">
        <input v-model="searchQuery" type="text" placeholder="Cari nama atau NIP guru..." class="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-4 py-2 text-xs text-slate-800 shadow-2xs focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition-colors" @input="onSearchInput" />
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <button @click="showForm = true; editing = null" class="btn btn-primary">
    ...
    <div class="bg-white rounded-xl shadow-2xs border border-slate-200 overflow-hidden">
      <div v-if="loading" class="text-center py-16">
        <div class="inline-flex w-8 h-8 border-4 border-slate-100 border-t-emerald-700 rounded-full animate-spin mb-4"></div>
        <p class="text-xs font-semibold text-slate-400">Memuat data guru...</p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50/90 border-b border-slate-200 text-slate-600">
              ...
              <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500">NAMA GURU</th>
              <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500">NIP</th>
              <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-500">MAPEL & KELAS DIAMPU</th>
          ...
          <tbody class="divide-y divide-slate-100 text-xs text-slate-800">
            <tr v-for="(row, index) in teachers" :key="row.id" class="hover:bg-slate-50/80 transition-colors">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2.5">
                  ...
                  <div class="w-8 h-8 rounded-lg overflow-hidden bg-slate-100 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 border border-slate-200 shadow-2xs">
                    ...
                    <div v-else class="w-full h-full bg-emerald-800 flex items-center justify-center">
                      {{ (row.full_name || row.user?.name || '?').split(' ').slice(0, 2).map(n => n[0]).join('').toUpperCase() }}
                    </div>
                  </div>
                  <div>
                    <span class="text-xs font-bold text-slate-900 block leading-tight">{{ row.full_name || row.user?.name || '-' }}</span>
                    <span v-if="row.position" class="inline-block mt-0.5 px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-medium rounded border border-slate-200/60">
                      {{ row.position }}
                    </span>
                  </div>
                </div>
              </td>
              <td class="px-4 py-3 text-xs font-semibold text-slate-600 font-mono tabular-nums">{{ row.nip || '-' }}</td>
              <td class="px-4 py-3">
                <div v-if="getSubjectClassGrouped(row).length > 0" class="flex flex-wrap gap-1.5">
                  <div v-for="grp in getSubjectClassGrouped(row)" :key="grp.subject_name" class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-50 border border-emerald-200/60 text-xs">
                    <span class="font-semibold text-emerald-900">{{ grp.subject_name }}</span>
                    <span class="font-medium text-slate-600 bg-white px-1 py-0.2 rounded border border-slate-200 text-[10px]">{{ grp.class_names }}</span>
                  </div>
                </div>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-1">
                  ...
                  <button @click="viewDetail(row)" title="Lihat Detail" class="w-7 h-7 rounded-lg flex items-center justify-center bg-white text-slate-600 hover:text-emerald-700 hover:border-emerald-200 border border-slate-200 transition-colors shadow-2xs cursor-pointer">
                    <Eye class="w-3.5 h-3.5" />
                  </button>
                  <button @click="resetPassword(row)" title="Reset Password" class="w-7 h-7 rounded-lg flex items-center justify-center bg-white text-slate-600 hover:text-amber-600 hover:border-amber-200 border border-slate-200 transition-colors shadow-2xs cursor-pointer">
                    <Key class="w-3.5 h-3.5" />
                  </button>
                  <button @click="edit(row)" title="Edit" class="w-7 h-7 rounded-lg flex items-center justify-center bg-white text-slate-600 hover:text-emerald-700 hover:border-emerald-200 border border-slate-200 transition-colors shadow-2xs cursor-pointer">
                    <Pencil class="w-3.5 h-3.5" />
                  </button>
                  <button @click="remove(row)" title="Hapus" class="w-7 h-7 rounded-lg flex items-center justify-center bg-white text-slate-600 hover:text-rose-600 hover:border-rose-200 border border-slate-200 transition-colors shadow-2xs cursor-pointer">
