<template>
  <div class="space-y-6 font-inter pb-12 max-w-7xl mx-auto">
    <!-- 1. FRESH VIBRANT EMERALD HERO BANNER (Waka Kurikulum) -->
    <div class="relative bg-gradient-to-r from-emerald-600 via-emerald-700 to-teal-800 text-white rounded-3xl shadow-xl shadow-emerald-700/15 overflow-hidden border border-emerald-500/40">
      ...
      <!-- Banner Content -->
      <div class="relative z-10 p-6 sm:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div class="flex flex-col sm:flex-row sm:items-center gap-5">
          <!-- Photo Frame -->
          <div class="w-20 h-20 sm:w-24 sm:h-24 bg-white/15 backdrop-blur-md rounded-2xl border-2 border-white/30 p-1 flex items-center justify-center flex-shrink-0 overflow-hidden relative shadow-lg">
            <img
              v-if="userPhoto"
              ...
              alt="Foto Profil"
            />
            <div v-else class="w-full h-full rounded-xl bg-emerald-800 flex items-center justify-center text-white font-black text-3xl uppercase">
        ...
        <div class="flex flex-wrap lg:flex-col xl:flex-row gap-2.5 flex-shrink-0">
          <RouterLink
            to="/admin/schedules"
            class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-amber-400 hover:bg-amber-300 text-slate-950 shadow-md transition-all active:scale-95 cursor-pointer"
          >
          ...
          <RouterLink
            to="/admin/grades"
            class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-md transition-all active:scale-95 shadow-xs cursor-pointer"
          >
            <Award class="w-4 h-4" />
          ...
          <RouterLink
            to="/admin/profile"
            class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-teal-900/80 hover:bg-teal-900 text-white border border-teal-400/40 backdrop-blur-md transition-all active:scale-95 shadow-xs cursor-pointer"
          >
    ...
    <!-- 2. QUICK BENTO METRIC CARDS -->
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5 sm:gap-4">
      <RouterLink to="/admin/schedules" class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-blue-400 hover:shadow-md transition-all group">
        ...
        <p class="text-2xl font-black text-slate-900 mt-2 font-lexend">{{ stats.schedules_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5">Sesi KBM terjadwal</p>
      </RouterLink>

      <RouterLink to="/admin/subjects" class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-indigo-400 hover:shadow-md transition-all group">
        <div class="flex items-center justify-between">
        ...
        <p class="text-2xl font-black text-slate-900 mt-2 font-lexend">{{ stats.subjects_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5">Kurikulum madrasah</p>
      </RouterLink>

      <RouterLink to="/admin/teachers" class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-teal-400 hover:shadow-md transition-all group">
        <div class="flex items-center justify-between">
        ...
        <p class="text-2xl font-black text-slate-900 mt-2 font-lexend">{{ stats.teachers_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5">Tenaga pengajar aktif</p>
      </RouterLink>

      <RouterLink to="/admin/classes" class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-amber-400 hover:shadow-md transition-all group">
        <div class="flex items-center justify-between">
        ...
        <p class="text-2xl font-black text-slate-900 mt-2 font-lexend">{{ stats.classes_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5">Kelas terdaftar</p>
      </RouterLink>

      <RouterLink to="/kurikulum/letters" class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-emerald-400 hover:shadow-md transition-all group col-span-2 sm:col-span-1">
        <div class="flex items-center justify-between">
        ...
        <p class="text-2xl font-black text-slate-900 mt-2 font-lexend">{{ stats.total_letters || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5">Surat Masuk & Keluar</p>
      </RouterLink>
    </div>

    <!-- 3. LIVE KBM SESSION MONITOR (Sedang Berlangsung Sekarang) -->
    <div v-if="selectedDay === currentTodayDay && currentLiveSlot" class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl p-5 sm:p-6 shadow-lg border border-slate-700/50 space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-700/60 pb-3.5">
        <div class="flex items-center gap-2.5">
          <span class="relative flex h-3 w-3">
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span class="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span>
          </span>
          <h3 class="text-sm sm:text-base font-black tracking-wide uppercase font-lexend flex items-center gap-2">
            <span>KBM SEDANG BERLANGSUNG SAAT INI</span>
            <span class="px-2.5 py-0.5 rounded-lg text-xs font-mono font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
              {{ currentLiveSlot.timeSlot }} WIB
            </span>
          </h3>
        </div>
        <p class="text-xs text-slate-400 font-medium">Status Real-Time Jam Pelajaran Hari Ini</p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <div
          ...
          :class="[
            ...
            'p-3.5 rounded-2xl border flex items-center justify-between gap-3 shadow-inner'
          ]"
        >
          <div class="space-y-1 min-w-0">
            <div class="flex items-center gap-2">
              <span class="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                {{ item.class_room?.name || item.classRoom?.name || 'Semua Kelas' }}
              </span>
              <span v-if="item.is_activity" class="text-xs font-bold text-amber-300">
                ⭐ {{ item.activity_name }}
              </span>
            ...
            <p v-if="!item.is_activity && item.teacher" class="text-xs text-slate-300 truncate flex items-center gap-1.5">
              <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
              <span>Guru: <strong>{{ item.teacher?.full_name }}</strong></span>
            </p>
          </div>
          ...
          <a
            ...
            target="_blank"
            class="p-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl transition-all shadow-md flex-shrink-0 cursor-pointer"
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden space-y-6 p-6 sm:p-7">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div class="space-y-1">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h2 class="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2 font-lexend">
              ...
              <p class="text-xs text-slate-500 font-normal">
                Alur kronologis jam mengajar guru, istirahat, dan kegiatan madrasah per kelas.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2.5 flex-wrap">
          <!-- View Switcher: Timeline vs Matriks Rombel -->
          ...
          <RouterLink
            to="/admin/schedules"
            class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors shadow-2xs cursor-pointer"
          >
            <Calendar class="w-3.5 h-3.5 text-slate-500" />
            <span>Kelola Master &rarr;</span>
          </RouterLink>
        </div>
      </div>
      ...
      <!-- Loading State -->
      <div v-if="loadingSchedules" class="py-16 text-center text-slate-400 space-y-2">
      ...
      <!-- Empty State -->
      <div v-else-if="filteredGroupedSchedules.length === 0" class="py-14 text-center text-slate-400 space-y-3 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
      ...
      <!-- VIEW MODE A: TIMELINE CHRONOLOGICAL VIEW -->
      <div v-else-if="viewMode === 'timeline'" class="space-y-6">
        <div
          ...
          class="space-y-3"
        >
          <!-- SPECIAL BREAK / PRAYER BANNER (Jika Semua Item di Slot Ini Merupakan Istirahat / Sholat) -->
          <div
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p :class="[getSlotBannerStyle(group).subColor, 'text-xs font-medium']">
                  Berlaku untuk seluruh siswa dan dewan guru madrasah
                </p>
              </div>
            </div>

            <div class="flex items-center gap-2">
              <span :class="[getSlotBannerStyle(group).timeBadge, 'px-3 py-1 rounded-xl text-xs font-mono font-bold tracking-tight shadow-2xs']">
                ⏱️ {{ group.timeSlot }} WIB
              </span>
              <span class="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-white/70 text-slate-700 border border-slate-200/60">
                Semua Kelas
              </span>
            </div>
          </div>
            ...
            <div class="flex items-center gap-3 pt-1">
              <div
                :class="[
                  ...
                  'inline-flex items-center gap-2 px-3 py-1 rounded-xl text-xs font-bold font-mono tracking-tight shadow-xs transition-colors'
                ]"
              >
                <Clock class="w-3.5 h-3.5" :class="isSlotLive(group) ? 'text-amber-300 animate-spin' : 'text-emerald-400'" />
                <span>{{ group.timeSlot }} WIB</span>
                <span v-if="isSlotLive(group)" class="text-[10px] font-sans font-black bg-amber-400 text-slate-950 px-1.5 py-0.2 rounded-md">LIVE</span>
              </div>
              <div class="h-px bg-slate-200 flex-1"></div>
              <span class="text-[11px] font-semibold text-slate-400">
                {{ group.items.length }} Rombel Belajar
              </span>
            </div>
      ...
      <div v-else-if="viewMode === 'matrix'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">
        <div
          v-for="cls in filteredClassList"
          :key="'matrix-cls-'+cls.id"
          class="bg-slate-50/70 rounded-3xl border border-slate-200/80 overflow-hidden shadow-xs space-y-3 p-4"
        >
          <!-- Class Column Header -->
          <div class="flex items-center justify-between pb-3 border-b border-slate-200">
            <div class="flex items-center gap-2">
              <div class="w-8 h-8 rounded-xl bg-emerald-600 text-white font-black text-xs flex items-center justify-center shadow-xs">
                {{ cls.name.charAt(0) }}
              </div>
              <h3 class="text-sm font-black text-slate-900 font-lexend">Kelas {{ cls.name }}</h3>
            </div>
            <span class="text-[10px] font-bold px-2 py-0.5 bg-white border border-slate-200 rounded-md text-slate-500">
          ...
          <div class="space-y-2.5">
            ...
            >
              <div class="flex items-center justify-between text-[11px]">
                <span class="font-mono font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md">
                  {{ sch.start_time?.substring(0, 5) }} - {{ sch.end_time?.substring(0, 5) }}
                </span>
                <span v-if="sch.is_activity" class="text-[10px] font-black text-amber-800 bg-amber-200/80 px-2 py-0.5 rounded-md">
                  KEGIATAN
                </span>
                <span v-else class="text-[10px] font-black text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-md">
                  {{ sch.subject?.code || 'MAPEL' }}
                </span>
              </div>
              ...
              <div>
                ...
                <p v-if="!sch.is_activity && sch.teacher" class="text-[11px] text-slate-500 truncate mt-0.5">
                  👤 {{ sch.teacher?.full_name }}
                </p>
              </div>
            </div>

            <div v-if="getClassSchedules(cls.id).length === 0" class="py-8 text-center text-slate-400 text-xs font-medium">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-base font-bold text-slate-900 font-lexend">Distribusi Jadwal Pelajaran</h3>
            <p class="text-xs text-slate-500">Pengaturan matriks jadwal KBM harian per kelas.</p>
          </div>
      ...
      <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-base font-bold text-slate-900 font-lexend">Kalender Akademik Madrasah</h3>
            <p class="text-xs text-slate-500">Agenda kegiatan akademik semester berjalan.</p>
          </div>
