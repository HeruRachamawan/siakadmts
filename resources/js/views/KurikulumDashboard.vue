<template>
  <div class="space-y-5 sm:space-y-6 font-inter pb-16 max-w-7xl mx-auto px-1 sm:px-2">
    ...
    <!-- 2. QUICK METRIC CARDS (Enterprise Standard) -->
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
      <RouterLink to="/admin/schedules" class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-slate-300 transition-colors">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">Jadwal Aktif</span>
          <div class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0">
            <Calendar class="w-4 h-4" />
          </div>
        </div>
        <p class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 font-mono tabular-nums">{{ stats.schedules_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5 truncate">Sesi KBM aktif</p>
      </RouterLink>

      <RouterLink to="/admin/subjects" class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-slate-300 transition-colors">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">Mata Pelajaran</span>
          <div class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center flex-shrink-0">
            <BookOpen class="w-4 h-4" />
          </div>
        </div>
        <p class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 font-mono tabular-nums">{{ stats.subjects_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5 truncate">Mapel kurikulum</p>
      </RouterLink>

      <RouterLink to="/admin/teachers" class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-slate-300 transition-colors">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">Guru Pengampu</span>
          <div class="w-8 h-8 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center flex-shrink-0">
            <Users class="w-4 h-4" />
          </div>
        </div>
        <p class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 font-mono tabular-nums">{{ stats.teachers_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5 truncate">Guru aktif</p>
      </RouterLink>

      <RouterLink to="/admin/classes" class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-slate-300 transition-colors">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">Rombel Kelas</span>
          <div class="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center flex-shrink-0">
            <Building2 class="w-4 h-4" />
          </div>
        </div>
        <p class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 font-mono tabular-nums">{{ stats.classes_count || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5 truncate">Kelas terdaftar</p>
      </RouterLink>

      <RouterLink to="/kurikulum/letters" class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-slate-300 transition-colors col-span-2 sm:col-span-1">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">Agenda Surat</span>
          <div class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center flex-shrink-0">
            <FileText class="w-4 h-4" />
          </div>
        </div>
        <p class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-1 font-mono tabular-nums">{{ stats.total_letters || 0 }}</p>
        <p class="text-[11px] text-slate-400 mt-0.5 truncate">Surat Masuk & Keluar</p>
      </RouterLink>
    </div>
