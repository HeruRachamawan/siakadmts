<template>
  <div id="beranda" class="min-h-screen bg-slate-50 font-inter text-slate-800 overflow-x-hidden selection:bg-emerald-500 selection:text-white">
    ...
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16 sm:h-20">
          ...
          <div class="flex items-center gap-3 group cursor-pointer">
            ...
            >
              <img v-if="settings.app_logo" :src="getStorageUrl(settings.app_logo)" class="w-full h-full object-contain" alt="Logo" />
              <BookOpen v-else class="w-5 h-5" />
            </div>
            <span
              class="font-extrabold text-base sm:text-lg tracking-wider uppercase truncate max-w-[200px] sm:max-w-none transition-colors"
          ...
          <div class="hidden lg:flex space-x-1 xl:space-x-2">
            <a
              href="#beranda"
              class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 hover:-translate-y-0.5"
              :class="scrolled ? 'text-slate-600 hover:text-emerald-700 hover:bg-emerald-50' : 'text-emerald-200 hover:text-white hover:bg-emerald-900/60'"
            >
              Beranda
            </a>
            <a
      ...
      >
        <div v-if="isMobileMenuOpen" class="lg:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl shadow-xl border-t border-slate-200 py-4 px-4 flex flex-col space-y-2">
          <a href="#beranda" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Beranda</a>
          <a href="#profil" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Profil & Visi Misi</a>
          <a href="#guru" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Dewan Guru</a>
          <a href="#wali-kelas" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Wali Kelas</a>
          <a href="#prestasi" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Prestasi</a>
          <a href="#berita" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Berita & Informasi</a>
          <a href="#fasilitas" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Sarana & Prasarana</a>
          <a href="#galeri" @click="closeMobileMenu" class="text-sm font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-2 rounded-lg transition-colors">Galeri</a>
    ...
    <!-- 1. Hero Section (Deep Emerald Dark Luxury with Beams & Ticker) -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
          ...
          <div class="lg:col-span-5 flex flex-col justify-center">
            <div
              ...
              class="interactive-tilt-card bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-xs relative overflow-hidden group hover:border-emerald-300 hover:shadow-lg transition-all duration-300"
            >
              ...
              <p class="text-slate-600 leading-relaxed text-xs sm:text-sm whitespace-pre-line border-t border-slate-100 pt-4">
                {{ settings.principal_description || 'Selamat datang di website resmi MTs Al - Hasanah. Sebagai institusi pendidikan, kami berkomitmen untuk tidak hanya fokus pada pencapaian akademik, tetapi juga pembangunan karakter islami dan kompetensi era digital. Mari bersama-sama wujudkan madrasah unggulan yang melahirkan bibit-bibit pemimpin masa depan.' }}
              </p>
            </div>
          </div>
                ...
                <div class="space-y-1 w-full">
                  ...
                  <ul v-else class="space-y-2.5 text-xs sm:text-sm text-slate-600">
                    <li class="flex items-start gap-2.5">
                      <span class="w-1.5 h-1.5 rounded-full bg-emerald-600 mt-2 flex-shrink-0"></span>
                      <span class="leading-relaxed">Menyelenggarakan proses pembelajaran yang inovatif, interaktif, dan berbasis pada nilai-nilai keislaman.</span>
                    </li>
                    <li class="flex items-start gap-2.5">
                      <span class="w-1.5 h-1.5 rounded-full bg-emerald-600 mt-2 flex-shrink-0"></span>
                      <span class="leading-relaxed">Membekali peserta didik dengan pemanfaatan teknologi digital yang produktif dan bertanggung jawab.</span>
                    </li>
                  </ul>
                </div>
              </div>
    ...
    <!-- 3. Latest News Section (White Canvas) -->
    <section id="berita" class="py-20 bg-white relative border-t border-slate-200/80 font-inter">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div class="space-y-1.5">
            ...
            <h2 class="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">Berita & Informasi Terkini</h2>
            <p class="text-slate-600 text-xs sm:text-sm font-normal">Ikuti berbagai kegiatan, pengumuman, dan prestasi terbaru dari madrasah kami.</p>
          </div>
        </div>

        <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="i in 3" :key="i" class="rounded-2xl border border-slate-200/80 p-4 h-[380px] animate-pulse bg-slate-50"></div>
        </div>
        <div v-else-if="posts.length === 0" class="text-center py-16 bg-slate-50/60 rounded-2xl border border-slate-200/80">
          <p class="text-slate-500 text-xs font-normal">Belum ada berita yang dipublikasikan.</p>
        </div>
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <!-- News Card with 3D Tilt -->
          <div
            v-for="post in posts"
            :key="post.id"
            ...
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col cursor-pointer"
          >
            <div class="h-44 bg-slate-100 relative overflow-hidden">
              <img v-if="post.image" :src="getStorageUrl(post.image)" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="News Cover" />
              <div v-else class="w-full h-full bg-emerald-50/50 flex items-center justify-center">
                <Newspaper class="w-10 h-10 text-emerald-400 opacity-60" />
              </div>
            </div>
            <div class="p-5 flex flex-col flex-grow space-y-2.5">
              <div class="text-[11px] font-semibold text-emerald-700">{{ formatDate(post.created_at) }}</div>
              <h3 class="text-base font-bold text-slate-900 leading-snug group-hover:text-emerald-700 transition-colors line-clamp-2">{{ post.title }}</h3>
              <p class="text-slate-600 text-xs line-clamp-3 leading-relaxed flex-grow font-normal">{{ post.content }}</p>
              <div class="pt-2 border-t border-slate-100">
                <span class="text-xs font-semibold text-emerald-700 flex items-center gap-1.5 group-hover:text-emerald-800">
    ...
    <!-- 4. Dewan Guru Section with Live Search Filter -->
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div class="space-y-2">
            ...
            <p class="text-slate-600 text-xs sm:text-sm font-normal">Mengenal lebih dekat para pendidik berdedikasi yang membimbing siswa-siswi menjadi generasi unggul.</p>
          </div>
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
          <div
            ...
            :key="teacher.id"
            @click="openLightbox(teacher, 'teacher')"
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col items-center p-5 text-center cursor-pointer"
          >
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          <div
            ...
            :key="classroom.id"
            @click="openLightbox(classroom.homeroom_teacher, 'teacher')"
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all duration-300 p-4 flex items-center gap-3.5 cursor-pointer"
          >
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div
            ...
            :key="achievement.id"
            @click="openLightbox(achievement, 'achievement')"
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col p-5 cursor-pointer"
          >
    ...
    <!-- 7. Facilities Section (White Canvas) -->
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
            ...
            :key="item.id"
            @click="openLightbox(item, 'facility')"
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col cursor-pointer"
          >
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          <div
            ...
            :key="item.id"
            @click="openLightbox(item, 'gallery')"
    ...
    <section id="lokasi" class="py-20 bg-white text-slate-900 border-t border-slate-200/80 relative overflow-hidden font-inter">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div
            @mousemove="handleCardTilt"
            @mouseleave="resetCardTilt"
            class="interactive-tilt-card lg:col-span-5 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-6 hover:border-emerald-300 transition-all duration-300"
          >
            <div class="space-y-6">
              <!-- School Header -->
          ...
          <div class="lg:col-span-7 rounded-2xl overflow-hidden min-h-[380px] shadow-xs border border-slate-200/80 relative bg-slate-100">
            <iframe
              v-if="cleanMapsUrl"
              :src="cleanMapsUrl"
    ...
    <!-- 10. Deep Emerald Footer (Simetris & Kokoh) -->
    ...
    <!-- Lightbox Modal with Smooth Scale Animation -->
    <Transition
    ...
    >
      <div v-if="activeLightbox" class="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 sm:p-6" @click.self="closeLightbox">
        <div class="relative w-full max-w-4xl bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col md:flex-row shadow-2xl animate-scale-up">
          ...
          <div class="md:w-2/3 bg-slate-950 flex items-center justify-center min-h-[280px] md:min-h-[460px]">
          ...
          <div class="md:w-1/3 p-6 md:p-8 flex flex-col max-h-[50vh] md:max-h-[85vh] overflow-y-auto custom-scrollbar text-slate-900 bg-white">
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <div class="flex flex-wrap gap-1.5">
                    ...
                    <span v-if="!activeLightbox.subjects || activeLightbox.subjects.length === 0" class="text-xs text-slate-400">Belum ada mapel</span>
                  </div>
                </div>
            ...
            <template v-else>
              <div class="text-xs font-bold text-emerald-700 mb-2">{{ formatDate(activeLightbox.created_at) }}</div>
              <h3 class="text-xl font-bold text-slate-900 mb-4">{{ activeLightbox.title }}</h3>
              <div class="text-xs text-slate-600 leading-relaxed">
                <p class="whitespace-pre-line">{{ activeLightbox.description || activeLightbox.content }}</p>
              </div>
            </template>
          </div>
        </div>
...
import { api } from '../../api';
import ImageSliderCard from '../../components/ImageSliderCard.vue';
...
function openLightbox(item, type = 'general') {
